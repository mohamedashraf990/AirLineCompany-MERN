...
const stripe = require("stripe")("sk_test_51K8pKeAHoHtEwtN543KDMLYmWqGvWFHQIlpTJKuuoq74xsRGb8K0ND1KkoICtgAODROeWZayMhWDqgRPqTGU7b4C00kJjZnbuv")
...
app.post("/payment", (req, res) => {
  ...
  const sender = {
    ...
    text: "You have Succefully made a reservation with the following details For the Departure Flight ( Flight Number :" + SelectedFlightNumber +
      "  Arrival Time : " + SelectedArrivalTime + "  Departure Date: " + SelectedDepartureDate + "  Arrival Terminal : " + SelectedArrivalTerminal + "  Departure Terminal: " + SelectedDepartureTerminal
      + " Economy Seats: " + SelectedEconomySeats + "  Business Class Seats: " + SelectedBusinessClassSeats + "  This Flight Price is " + SelectedFlight.TotalPrice + " You have Succefully made a reservation with the following details For the Arrival Flight ( Flight Number :" + PrevFlightNumber +
      "  Arrival Time : " + PrevArrivalTime + "  Departure Date: " + PrevDepartureDate + "  Arrival Terminal : " + PrevArrivalTerminal + "  Departure Terminal: " + PrevDepartureTerminal
      + " Economy Seats: " + PrevEconomySeats + "  Business Class Seats: " + PrevBusinessClassSeats + "  This Flight Price is " + PrevFlights.TotalPrice
  }
...
app.post("/paymentChange", (req, res) => {
  ...
  const sender = {
    ...
    text: "You have Succefully made a reservation with the following details For the Departure Flight ( Flight Number :" + SelectedFlightNumber +
      "  Arrival Time : " + SelectedArrivalTime + "  Departure Date: " + SelectedDepartureDate + "  Arrival Terminal : " + SelectedArrivalTerminal + "  Departure Terminal: " + SelectedDepartureTerminal
      + " Economy Seats: " + SelectedEconomySeats + "  Business Class Seats: " + SelectedBusinessClassSeats + "  This Flight Price is " + SelectedFlight.TotalPrice
  }
...
app.post("/registration", async (req, res) => {
  ...
  if (FirstName == "" || LastName == "" || Email == "" || Password == "" ||
    PhoneNumber == "" || PassportNumber == "" || CountryCode == ""
...
app.put('/EditDepartureFlight', (req, res) => {
  ...
  Reservation.updateOne({ FlightNumber: FlightNumber }, EditedObj).clone().then(result => {
      ...
      bcrypt.compare(Password1, result[0].Password).then((result2) => {
        if (result2 == false) {
          ...
          if (UserName != 'admin') {
            ...
            const accessToken = jwt.sign(user, process.env.ACCESS_TOKEN_SECRET)
            //console.log(accessToken)
            res.send({ msg: "Admin verified", token: accessToken, isloggedin: true, isAdmin: true, UserName: UserName })
...
app.put('/changeEmail', authenticate, (req, res) => {
  ...
  User.updateOne({ Email: OldEmail }, newobjj,).clone().then(result => {
...
app.put('/changePassword2', async (req, res) => {
  ...
  User.updateOne({ Email: Email }, newobjj,).clone().then(result => {
    console.log(result)
    if (result.modifiedCount == 1) {
...
app.put('/changePassword', authenticate, async (req, res) => {
  ...
  User.updateOne({ UserName: username }, newobjj,).clone().then(result => {
    console.log(result)
    if (result.modifiedCount == 1) {
...
app.put('/updateuser', authenticate, (req, res) => {
  ...
  User.updateOne({ UserName: username }, updateObj2,).clone().then(result => {
    console.log(result)
    if (result.modifiedCount == 1) {
      res.send("User Updated")
    } else {
...
app.post('/emailme', (req, res) => {
  ...
  const sender = {
    ...
    to: mail,
    subject: "sending email from node js",
    text: " you have flight reservation with the following details (Flight Number :" + FlightNumber +
      "  Arrival Time : " + ArrivalTime + "  Departure Date: " + DepartureDate + "  Arrival Terminal : " + ArrivalTerminal + "  Departure Terminal: " + DepartureTerminal
      + " Economy Seats: " + EconomySeats + "  Business Class Seats: " + BusinessClassSeats + "  The tota; amount will be " + TotalPrice
  }
